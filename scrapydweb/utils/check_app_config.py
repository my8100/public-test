...
def check_scrapyd_servers(config):
    ...
    SCRAPYD_SERVERS_PUBLIC_URLS = config.get('SCRAPYD_SERVERS_PUBLIC_URLS', None) or [None] * len(SCRAPYD_SERVERS)
    ...
    for idx, (server, public_url) in enumerate(zip(SCRAPYD_SERVERS, SCRAPYD_SERVERS_PUBLIC_URLS)):
        ...
        public_url = public_url.strip(' /') if public_url is not None else None
